...
function reConfigCmtParam(n) {
  typeof oParams != "undefined" &&
    ...
    (n == 1
      ...
      : n == 2 &&
        ((oParams.sJsAjax = window.location.origin + "/commentmwg/ajax/index"),
        (oParams.sJsHome = window.location.origin + "/commentmwg")),
...
function initUploadRatingImg() {
  ...
  t == 1
    ? (n = getParam("sJsHomeU") + "/aj/Cmt/PostRatingImage")
    : t == 2 && (n = getParam("sJsHome") + "/aj/Home/PostImage");
  setTimeout(function () {
    rating_popup.find(".send-img").unbind();
    rating_popup.find(".send-img").on("click", function () {
      if (
        (console.log("up Img Cmt"),
        rating_popup.find(".resRtImg li").length > 2)
      )
        return alert("ÄĂ£ up load quĂ¡ sá»‘ áº£nh quy Ä‘á»‹nh. "), !1;
    ...
    rating_popup.find("#hdFileRatingUpload").html5Uploader({
      ...
      onServerLoadStart: function () {
        ...
        n +=
          "<img src='//cdn.tgdd.vn/mwgcart/mwg-site/ContentMwg/images/loading.gif' />";
      ...
      onSuccess: function (n) {
        ...
        getRtImg();
        uploadedFileMap.set(
          `uploadedFile${rating_popup.data("index")}`,
          uploadedFileMap.get(`uploadedFile${rating_popup.data("index")}`) + 1,
        );
      },
    });
  }, 1e3);
...
function getRtImg() {
  if (
    (console.log("getRtImg_mwg"), rating_popup.find(".resRtImg li").length > 0)
  ) {
    var n = "";
    ...
    rating_popup.find(".resRtImg li").length == 0 &&
...
function rtMoveFile(n, t) {
  ...
  r == 1
    ? (i = getParam("sJsHomeU") + "/aj/Cmt/MoveFileRatingImage")
    : r == 2 && (i = getParam("sJsHome") + "/aj/Home/MoveFileRatingImage");
  n != null &&
    n != "" &&
...
function reInitCmt2021() {
  if (!gl_getJsCmtDmx) {
    ...
    n == 2 &&
      ($(".midcmt .s_comment i")
        .removeClass("icondmx-search")
        .addClass("icon-search"),
      console.log("reInitCmt2021_"),
      (gl_getJsCmtDmx = !0));
    $(".rating-cmt-info-js").attr("data-jsOvrCmt") != null &&
      $(".rating-cmt-info-js").attr("data-jsOvrCmt") != "" &&
      $.getScript($(".rating-cmt-info-js").attr("data-jsOvrCmt")).done(
        function () {
  ...
  if (isMobile) {
    ...
    r.length == 0
      ...
      : r.length == 1 &&
        (n.addClass("one-cmt"),
        n
          .find(".slide-cmt.carousel")
          .addClass("box-cmt")
          .removeClass("slide-cmt")
          .removeClass("carousel"),
        r.find(".text-cmt").append(r.find(".rating-product__star")));
  } else {
    let r = $(".slide-cmt");
    ...
    u.length < f
      ? (u.each(function () {
          ...
          n.find(".text-cmt").after(t.addClass("flex"));
          n.find(".rating-product__star li i")
            .removeClass("iconratingnew-star--medium")
            .addClass("iconratingnew-star--big");
        }),
        (r.data("owl.carousel").options.items = u.length))
      : u.length == f &&
        ((r.data("owl.carousel").options.stagePadding = 0),
        r.removeClass("stage-padding"));
...
function initRating() {
      ...
      function () {
        ...
        for (
          $(".selStr i").removeClass("active"),
            t = parseInt($(this).attr("id").replace("ss", "")),
            n = 0;
          n <= t;
          n++
        )
          $("#ss" + n).addClass("active");
      },
      function () {
        $(".selStr i").removeClass("active");
      },
  ...
  rating_popup.find(".ul-orslt li .btn-assess").click(function () {
    ...
    rating_popup
      .find(".criteriaID" + n + " .btn-assess")
      .removeClass("checkact");
  ...
  $(".txt-agree").click(function () {
    ...
    n === 1
      ? $("#hdfIsShare").val(0)
      : n === 0 &&
        ($("#hdfIsShare").val(1), $(".txt-agree").addClass("selected"));
  });
  $(".icon-dots").unbind();
...
function initRtSlide() {
  if (gl_rt_isMobile)
    console.log("slide m_111"),
      $(".show-comment-main").owlCarousel({
        lazyLoad: !0,
        items: 1,
        loop: !0,
        dots: !1,
...
function getOverrideScript(n) {
  ...
  gl_rt_isRatingPage == 1
    ? typeof initUploadRatingImg == "function" &&
      (console.log("getOverrideScript jsOvrCmt_detail"),
      initUploadRatingImg(),
      gl_isGetScriptMap.set(`gl_isGetScript${n}`, !0))
    : (typeof initUploadRatingImg == "function" &&
        (console.log("getOverrideScript jsOvrCmt_detail"),
        initUploadRatingImg()),
      gl_isGetScriptMap.set(`gl_isGetScript${n}`, !0));
...
function updateCKLikeCmt(n) {
  var i = "tgdd_cmtlike",
    t;
  if (
    (gl_rt_siteID == 2 && (i = "dmx_cmtlike"),
    (t = getCookie(i)),
    t !== undefined && t !== null && t !== "")
  ) {
    for (var u = !0, r = 0, f = t.split("-"), r = 0; r < f.length; r++)
      parseInt(n) === parseInt(f[r]) && (u = !1);
  ...
  if (
    ...
    for (n = 0, i = t.split("-"), n = 0; n < i.length; n++)
      rating_popup.find("#r-" + i[n]).length > 0 &&
        (rating_popup
          .find("#r-" + i[n] + " .click-like")
          .attr("href", "javascript:;"),
        rating_popup
          .find("#r-" + i[n] + " .click-like")
          .html("<i class='icondetail-like'></i> Há»¯u Ă­ch (1)"));
}
          ...
          try {
            ...
            if (t.res == 1) {
              parseInt(t.resCmt) > 0 &&
                i != null &&
                i != "" &&
                (console.log("rtMoveFile: " + i),
                rtMoveFile(i, t.resCmt),
                hideloading());
              ...
              dataLayer.push(f);
              t.popupSuccess != ""
                ? (gl_rt_isMobile && $(".locationbox_overlayaccess").show(),
                  $(".rating-product").after(t.popupSuccess))
                : (alert(
                    "ÄĂ¡nh giĂ¡ cá»§a báº¡n sáº½ Ä‘Æ°á»£c há»‡ thá»‘ng kiá»ƒm duyá»‡t. Xin cĂ¡m Æ¡n.",
                  ),
                  $(".locationbox_overlayaccess").hide());
...
function ratingRelply(n) {
  ...
  if (
    ...
    return (
      (gl_sendRating = !1),
      alert("Vui lĂ²ng nháº­p ná»™i dung cáº§n tháº£o luáº­n"),
      !1
    );
  if (
    ((r = "tgdd_fullname"),
    gl_rt_siteID == 2 && (r = "dm_fullname"),
    (t = getCookie(r)),
    t == null || t == "")
  )
    return (gl_sendRating = !1), showReplyConfirmPopup(), !1;
  u = {
  ...
  $.ajax({
    ...
    success: function (n) {
      if (n != null || n != "")
        try {
          n.res == 1 &&
            ($(".ratingLst .reply input").val(""),
            alert(
              "Tháº£o luáº­n cá»§a báº¡n sáº½ Ä‘Æ°á»£c há»‡ thá»‘ng kiá»ƒm duyá»‡t. Xin cĂ¡m Æ¡n.",
            ));
          gl_sendRating = !1;
        } catch (t) {}
      hideloading();
...
function validateRating() {
  var n, i, t;
  return (clearBorder(),
  rating_popup.find(".lbMsgRt").removeClass("hide"),
  (n = $.trim(rating_popup.find(".input [name=fRContent]").val())),
  (i = parseInt(rating_popup.find("#hdfStar").val())),
  i == 0)
    ? (rating_popup
        .find(".lbMsgRt")
        .html("Báº¡n chÆ°a Ä‘Ă¡nh giĂ¡ Ä‘iá»ƒm sao, vui lĂ²ng Ä‘Ă¡nh giĂ¡."),
      paddingForm(),
      !1)
    : (n == null || n == "") &&
      confirm(
        "Chá» Ä‘Ă£! Ná»™i dung chÆ°a Ä‘Æ°á»£c nháº­p, báº¡n sáºµn lĂ²ng nháº­p thĂªm chá»©?",
      )
    ? (rating_popup.find(".frtip .ct").focus(), !1)
    : ((n = $.trim(rating_popup.find(".input [name=fRName]").val())),
      n == null || n == "")
    ? (rating_popup.find(".lbMsgRt").html("Vui lĂ²ng nháº­p há» tĂªn"),
      rating_popup.find(".input [name=fRName]").addClass("borderWn"),
      paddingForm(),
      !1)
    : ((n = $.trim(rating_popup.find(".input [name=fRPhone]").val())),
      n == null || n == "")
    ? (rating_popup
        .find(".lbMsgRt")
        .html("Vui lĂ²ng nháº­p sá»‘ Ä‘iá»‡n thoáº¡i."),
      rating_popup.find(".input [name=fRPhone]").addClass("borderWn"),
      paddingForm(),
      !1)
    : rating_popup.find(".ul-orslt li").length > 0 &&
      ((t = rating_popup.find(".ul-orslt .checkact").length),
      t > 0 && t != rating_popup.find(".ul-orslt li").length)
    ? (rating_popup
        .find(".lbMsgRt")
        .html("Vui LĂ²ng chá»n táº¥t cáº£ cĂ¡c tiĂªu chĂ­"),
      paddingForm(),
      !1)
...
function paddingForm() {
  gl_rt_isMobile &&
    $(".read-assess-form").css(
      "padding-bottom",
      `${$(".submit-container").outerHeight()}px`,
    );
}
function rSelGender(n, t) {
  $(".rCfmInfo .cgd i")
    .removeClass("icondetail-radcheck")
    .addClass("icondetail-rad");
...
function countTxtRating() {
  var n = $(".frtip textarea").val().length,
    t;
  n > 0 && n < 80
    ? ((t = n + " kĂ½ tá»± (tá»‘i thiá»ƒu 80)"), $(".mintext").html(t))
    : $(".mintext").html("");
}
    ...
    t != null && t != ""
      ? (console.log("popuprating: " + t),
        ...
        n.length > 2 &&
          ...
          i != null &&
            i != "" &&
            (rating_popup.find(".wrap_fdback .input [name='fRName']").val(i),
            rating_popup.find(".wrap_fdback .input [name='fRPhone']").val(r))))
...
function ratingSearch() {
  var n, t, i, r;
  if (
    (console.log("ratingSearch0618_m"),
    (n = $(".cmtKey").val()),
    (n = $.trim(n)),
    n === null || n === "" || n.length < 3)
  )
  ...
  $.ajax({
    ...
    success: function (n) {
      (n !== null || n !== "") &&
        ...
        $(".rtPage .pgrc").remove(),
        gl_rt_isMobile
          ? $(".rtPage .boxsort").after(n)
          : $(".rtPage .content-wrap .rtFilter").after(n),
        (gl_sendRating = !1));
...
function getFileUpload() {
  gl_isFeedbackLoad ||
    $.getScript(
      "/lib/jquery-upload/jquery.html5uploader.min.js?v=20220613",
    ).done(function () {
      gl_isFeedbackLoad = !0;
      initUploadRating();
    });
      ...
      if (n != null || n != "")
        try {
          ...
          gl_rt_isMobile
            ? $(".rtPage .boxsort").after(n)
            : $(".rtPage .content-wrap .rtFilter").after(n);
          $(".rtPage .filter-choose").length > 0 &&
            ($(".rtPage .filter-choose a").removeClass("active"),
            $(".rtPage .filter-choose .rtF" + i).addClass("active"));
...
function reloadRatingUrl() {
  ...
  gl_rtCurStar !== null &&
    ...
    window.history.replaceState(
      "",
      "",
      updateURLParameter(window.location.href, "s", gl_rtCurStar),
    );
  gl_rt_currentImg !== null &&
    ...
    window.history.replaceState(
      "",
      "",
      updateURLParameter(window.location.href, "i", "1"),
    );
  gl_rt_currentOrder !== null &&
    gl_rt_currentOrder > 0 &&
    window.history.replaceState(
      "",
      "",
      updateURLParameter(window.location.href, "o", gl_rt_currentOrder),
    );
  gl_rt_currentPage !== null &&
    gl_rt_currentPage > 0 &&
    window.history.replaceState(
      "",
      "",
      updateURLParameter(window.location.href, "p", gl_rt_currentPage),
    );
}
...
function goToRSlide(n) {
  ...
  n > 0 &&
    (gl_rt_isMobile
      ? $(".show-comment-main").trigger("to.owl.carousel", n - 1)
      : $("#cmt_sync1").trigger("to.owl.carousel", n - 1));
}
function updateURLParameter(n, t, i) {
  var e = "",
    ...
    s;
  if (o)
    for (r = o.split("&"), u = 0; u < r.length; u++)
      r[u].split("=")[0] != t && ((e += f + r[u]), (f = "&"));
...
function removeURLParameter(n) {
  ...
  if (u.length >= 2) {
    var f = u.shift(),
      ...
      i = e.split(/[&;]/g);
    for (r = i.length; r-- > 0; )
      i[r].lastIndexOf(o, 0) !== -1 && i.splice(r, 1);
    t = f + "?" + i.join("&");
...
function showRatingForm(n, t) {
  ...
  rating_popup = rating_container.find(
    ".rating-product__popup[data-index=" + rating_block.data("index") + "]",
  );
  productUrl = null;
  ...
  gl_rt_isMobile
    ? rating_popup
        ...
        .removeClass("hide")
        .addClass("blockshow")
    : (rating_popup.find(".read-assess").removeClass("hide").addClass("showR"),
      $(".locationbox_overlayaccess").show());
  parseInt(n) > 0 &&
...
function hideInputRating() {
  ...
  rating_popup.find("#hdfStar").val() > 0
    ? confirm(
        "Chá» Ä‘Ă£! Báº¡n chÆ°a gá»­i Ä‘Ă¡nh giĂ¡, báº¡n cĂ³ cĂ³ muá»‘n gá»­i Ä‘i khĂ´ng?",
      )
      ? submitRatingComment()
      : closeRatingForm()
    : closeRatingForm();
...
function closeRatingForm() {
  ...
  rating_popup
    .find(".frtip .fsrt a")
    .attr("href", "javascript:showInputRating()");
  rating_popup
    .find(".frtip .fsrt a span")
    .html("Gá»­i Ä‘Ă¡nh giĂ¡ cá»§a báº¡n");
  gl_rt_isMobile
    ? rating_popup
        .find(".read-assess")
        .removeClass("blockshow")
        .addClass("hide")
    : (rating_popup.find(".read-assess").removeClass("showR").addClass("hide"),
      $(".locationbox_overlayaccess").hide());
}
function closeRtGallery() {
  ...
  $("body").css({ overflow: "" });
  $("html").css({ overflow: "" });
  gl_rt_isMobile || gl_rt_isRatingPage
    ? $(".locationbox_overlayaccess").length > 0 &&
      $(".locationbox_overlayaccess").hide()
    : $(".locationbox_overlayaccess").hide();
}
function showReplyConfirmPopup() {
  ...
  gl_rt_isMobile
    ? $(".rRepPopup").removeClass("hide").addClass("blockshow")
    : ($(".rRepPopup").removeClass("hide").addClass("blockshow"),
      $(".locationbox_overlayaccess").show());
}
...
function hideRtContentText() {
  ...
  rating_popup.find(".ratingLst .comment-content").each(function () {
    if (
      $(this).find(".cmt-txt").length > 0 &&
      $(this).find(".cmt-txt").html().length > n
    ) {
      $(this).find(".cmt-txt").addClass("hideRtCt");
      var t = $(this).parent().attr("id");
      $(this).after(
        "<span class='sRtXt' onclick='showFullRtCt(this)'>Xem tiáº¿p â–¾ </span>",
      );
    }
  });
  rating_popup.find(".show-comment-main .comment-content").each(function () {
    if (
      $(this).find(".cmt-txt").length > 0 &&
      $(this).find(".cmt-txt").html().length > 120
    ) {
      $(this).find(".cmt-txt").addClass("hideRtCt");
      var n = $(this).attr("data-id");
      $(this).after(
        '<span class="sRtXt sRtXt' +
          n +
          '" onclick="showFullRtCtPop(\'' +
          n +
          "')\">Xem tiáº¿p â–¾ </span>",
      );
    }
  ...
  n.each(function () {
    let n = $(this),
      t = n
        .closest(".daily-sg")
        .find(
          ".box-common__content:not([data-is-recommend-tab]):not([data-campaign]):not([data-group])",
        );
    t != null &&
      (n.data("is-recommend-tab") != null &&
        t.attr("data-is-recommend-tab", n.data("is-recommend-tab")),
  ...
  t.each(function () {
    let n = $(this),
      t = n
        .closest(".box-common")
        .find(
          ".box-common__content:not([data-cate-id]):not([data-prop-id]):not([data-prop-value-ids]):not([data-html-id]):not([data-product-ids]):not([data-url]):not([data-name]):not([data-frame])",
        );
    t != null &&
      (n.data("cate-id") != null && t.attr("data-cate-id", n.data("cate-id")),
      n.data("prop-id") != null && t.attr("data-prop-id", n.data("prop-id")),
      n.data("prop-value-ids") != null &&
        t.attr("data-prop-value-ids", n.data("prop-value-ids")),
      n.data("html-id") != null && t.attr("data-html-id", n.data("html-id")),
      n.data("product-ids") != null &&
        t.attr("data-product-ids", n.data("product-ids")),
...
function InitCarousel() {
  ...
  $(".prd-another .item a img").css(
    "height",
    `${Math.min(screen.width - 20, 640) / 3}px`,
  );
  $(".discount-payonl .scroll-box").owlCarousel({
    items: 1,
    loop: !1,
    dots: !0,
    margin: 0,
...
function InitOwlCarousel() {
  $(".discount-payonl .home-slider .slider-banner").owlCarousel({
    items: 1,
    loop: !1,
    dots: !1,
    margin: 0,
...
function InitInviteRatingOwlCarousel(n) {
  ...
  n.on("translate.owl.carousel", function () {
    setTimeout(function () {
      ...
      t.last().hasClass("active")
        ? n.removeClass("padding-right")
        : n.addClass("padding-right");
      t.first().hasClass("active")
        ? n.removeClass("padding-left")
        : (n.addClass("padding-left"),
          n.find(".owl-stage").css("padding-left", `${u * 2}px`));
    }, 1);
  }).owlCarousel({
    items: r,
    slideBy: "page",
...
function ShowHideStickyButton() {
  ...
  t.isInViewport()
    ? n.hide()
    : sessionStorage.getItem(keyStickyButton) == null && n.show();
}
  ...
  if (isRunLive && gbl_siteId == 2 && !1) {
    ...
    return (
      (document.cookie =
        ...
        (n ? 1 : 0) +
        ";expires=" +
        new Date(new Date().setHours(23, 59, 59, 999) - 252e5) +
        ";path=/"),
      n
    );
  }
...
function GetPredictedProduct() {
  ...
  $.ajax({
    ...
    success: function (t) {
      let i = $(t);
      i.find(".listproduct").length &&
        n.find(".box-common__content").html(i.children());
    },
    complete: function () {
      t.hide();
    },
...
function flashsaleCountdown() {
  ...
  if (
    ...
    i < "10" && (i = "0" + i),
    r < "10" && (r = "0" + r),
    parseInt(n) <= 0 && parseInt(i) <= 0 && parseInt(r) <= 0)
  ) {
...
function slickFS() {
  if (
    $(".slider-flashsale").data("mobile") != 1 &&
    $(".slider-flashsale .item").length
  )
...
function GetListCrmFS() {
  $(".flashsale-block .slider-flashsale .item").each(function () {
    var i = $(this).data("id"),
      ...
      t;
    n > 0 != !1 &&
      ((t = { ProductId: i, CrmProgramId: n, htmlViewResult: "" }),
      lstJsonModel.push(t));
  });
  $.ajax({
    url: "/HomeV2/GetCrmCountByProductList",
    data: {
      jsonModel: lstJsonModel,
      beginfsdatetime: $(".flashsale-block .endtime").data("begin"),
    },
    cache: !1,
    type: "POST",
...
function MoveSoldOutLayer(n) {
  ...
  i.hasClass("sold-out") &&
    (i.removeClass("sold-out"),
    n.addClass("sold-out"),
    n.find(".item-img").append(t.find(".layer-sold-out")));
}
var gl_getJsCmt = !1,
  gl_getJsCmtDmx = !1,
  gl_siteID = 1,
...
$(document).ready(function () {
  if ($(".rating-cmt-info-js").length != 0) {
    ...
    n == 2 &&
      ((gl_siteID = 2),
      (comment_cdn = "https://cdn.tgdd.vn/dienmay2015/comment"),
      (tgddc_urlroot = "//www.dienmayxanh.com/comment"));
...
$(document).ready(function () {
  ...
  if ((n.length > 0 && n.html(n.html().replaceAll("&nbsp;", "")), isMobile)) {
    ...
    n &&
      navigator.userAgent.indexOf("tgdd") > 0 &&
  ...
  $(".option-sg a").click(function () {
    ...
    if (i != null && i.length > 0)
      return (
        ...
        i.show(),
        i.find(".listproduct").addClass("tracking"),
        !1
      );
  ...
  $(document).on("click", ".box-common__content .showmore", function () {
    ...
    var f = $(".daily-sg .box-common__content")
      .eq(0)
      .find(".listproduct .item.hide > a");
    if (f.length > 0)
      addListProductIntoImpression(
        f,
        "Home page -  Gá»£i Ă½ hĂ´m nay - Cho báº¡n",
        10,
      );
    o.slice(i * u, (i + 1) * u).removeClass("hide");
    t.find(".item:not(.hide)").length == s
      ? (n.remove(), e.removeClass("hide"))
      : n.attr("data-page-index", i + 1);
  });
  $("ul.box-common__tab li").click(function () {
    ...
    if (n != null && n.length > 0)
      return (
        ...
        isMobile ||
          InitProductOwlCarousel(i.find(".listproduct.owl-carousel:visible")),
        n.show(),
    ...
    let a = {
      ...
      htmlId: f,
      propertyId: e,
      propertyValueIds: ConvertToArray(o),
      productIds: ConvertToArray(s),
      categoryUrl: h,
      categoryName: r,
      frame: c,
    };
    $.ajax({
      ...
      success: function (t) {
        ...
        l.hide();
        let a = "";
        i.parents().hasClass("promo-7day")
          ? (a = "Tuáº§n lá»… knockout")
          : i.parents().hasClass("promo-muanong") &&
            (a = "Tuáº§n lá»… gia dá»¥ng");
        r != null && (a += " - " + r);
        let v =
          n.find(".listproduct .active .item > a").length > 1
            ? n.find(".listproduct .active .item > a")
            : n.find(".listproduct li > a");
...
$(document).ready(function () {
  !1 && $(".slider-flashsale .item").length <= 0
    ...
    : ((intervalFS = setInterval(function () {
      ...
      slickFS(),
      GetListCrmFS(),
      window.location.hash == "#flashsale" &&
        $("html, body").animate(
          { scrollTop: $(".flashsale-block").offset().top },
          500,
        ));
    ...
    t.hasClass("active") ||
      ...
      $.ajax({
        ...
        success: function (n) {
          ...
          $(".listproduct.slider-flashsale")
            .removeClass("two-line")
            .removeClass("has-scroll");
          $(".listproduct.slider-flashsale").removeClass("has-slick");
          $(".listproduct.slider-flashsale").removeClass("flow-row");
          t > 3 &&
            ($(".listproduct.slider-flashsale").addClass("two-line"),
            t > 6 && $(".listproduct.slider-flashsale").addClass("has-scroll"));
